...
import { Download, Trash2, Coffee, Loader2, AlertTriangle, Search, ExternalLink, Play, CheckCircle2 } from 'lucide-react';
...
const ConvertedSites: React.FC = () => {
  ...
  // Helper function to render a site card
  const renderSiteCard = (site: ConvertedSite) => {
  ...
  // Determine which sites to display based on active tab
  const displaySites = () => {
    switch (activeTab) {
      case "in-progress":
        return inProgressSites;
      case "completed":
      ...
      {convertedSites?.length === 0 ? (
        ...
        <Tabs defaultValue="all" value={activeTab} onValueChange={setActiveTab} className="mb-6">
          ...
          <TabsContent value="all" className="mt-4">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {convertedSites?.map(renderSiteCard)}
            </div>
          </TabsContent>

          <TabsContent value="in-progress" className="mt-4">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {inProgressSites.map(renderSiteCard)}
            </div>
          </TabsContent>
          ...
          <TabsContent value="failed" className="mt-4">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {failedSites.map(renderSiteCard)}
            </div>
          </TabsContent>
        </Tabs>
      )}
